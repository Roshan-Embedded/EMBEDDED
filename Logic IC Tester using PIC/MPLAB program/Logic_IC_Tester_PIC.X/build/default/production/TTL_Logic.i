...
unsigned int i,j,k,n=9,l,x;
_Bool x1,x2,x3;
_Bool gate1[8]={0,0,0,0,0,0,0,0};
_Bool gate2[8]={0,0,0,0,0,0,0,0};
_Bool gate3[8]={0,0,0,0,0,0,0,0};
_Bool AND_3[8]={0,0,0,0,0,0,0,1};
_Bool OR_3[8]={0,1,1,1,1,1,1,1};
# 44 "./General.h"
void Mode_4(void);
void Write_IC(void);
...
void Auto(void);
void gate_1(void);
_Bool is_all_good(unsigned int n,unsigned int no);


void TRI_NAN(unsigned int ic_num);
_Bool TRI_GOOD(unsigned int ic_num);
# 6 "TTL_Logic.c" 2
...
void Mode_4(void)
{
    k=0;
    if(n==1){
    TRISD7=TRISD6=TRISD4=TRISD3=TRISB5=TRISB6=TRISB2=TRISB3=0;
    TRISB0=TRISD5=TRISB4=TRISB1=1;
    _delay((unsigned long)((20)*(16000000/4000.0)));
    for(i=0;i<2;i++)
    {
        for(j=0;j<2;j++)
        {
            RD7=RD4=RB5=RB2=(__bit)i;
            RD6=RD3=RB6=RB3=(__bit)j;
            k=(int)RB0;
            gate[0]*=10;
            gate[0]+=k;
            k=(int)RD5;
    ...
    TRISB0=TRISD7=TRISD5=TRISD4=TRISB5=TRISB4=TRISB2=TRISB1=0;
    TRISD6=TRISD3=TRISB6=TRISB3=1;
    _delay((unsigned long)((20)*(16000000/4000.0)));
    for(i=0;i<2;i++)
    {
        for(j=0;j<2;j++)
        {
            RB0=RD5=RB5=RB2=(__bit)i;
            RD7=RD4=RB4=RB1=(__bit)j;
            k=(int)RD6;
            gate[0]*=10;
            gate[0]+=k;
            k=(int)RD3;
...
{
    k=0;
    TRISB0=TRISD6=TRISD4=TRISB5=TRISB3=TRISB1=0;
    TRISD7=TRISD5=TRISD3=TRISB6=TRISB2=TRISB4=1;
    _delay((unsigned long)((20)*(16000000/4000.0)));
    for(i=0;i<2;i++)
    {
        RB0=RD6=RD4=RB5=RB3=RB1=(__bit)i;
...
void TRI_NAN(unsigned int ic_num){
    ...
    LCD_Set_Cursor(2,1);
    if(ic_num==4075){
        TRISB0=TRISD7=TRISD6=TRISD5=TRISD4=TRISB6=TRISB3=TRISB2=TRISB1=0;
        TRISB4=TRISB5=TRISD3=1;}
    else{
    TRISB0=TRISD7=TRISD6=TRISD5=TRISD4=TRISB5=TRISB4=TRISB3=TRISB1=0;
    TRISD3=TRISB6=TRISB2=1;}
    _delay((unsigned long)((50)*(16000000/4000.0)));
    for(i=0;i<2;i++){
        for(j=0;j<2;j++){
            for(k=0;k<2;k++){
...
_Bool TRI_GOOD(unsigned int ic_num){
    for(i=0;i<8;i++){
        if(ic_num==6){
            if(gate1[i]==gate2[i]==gate3[i]==AND_3[i]) return 0;
        }
        else if(ic_num==7){
            if(gate1[i]!=gate2[i]!=gate3[i]!=AND_3[i]) return 0;
        }
